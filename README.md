...
- **단기 추천 종목**: 가중치 점수 시스템 기반 평균 회귀 추천 (blended_target — 칼만 예측가+볼린저 SMA 혼합 목표가 — 기준 2% 이상 상승 여력 필터, 손절가/목표가 포함)
- **칼만 필터 추천 종목**: blended_target (칼만 예측가+볼린저 SMA 혼합) > 현재가 필터 기반 추가 단기 추천 (기존 추천과 중복 제외)
...
- **Ross Cameron 추천**: RSI < 30 + MACD 골든크로스 + MACD 양수 영역 조건 기반 Warrior Trading 전략 추천 Top 5 (일일·프리마켓 리포트 모두 포함). 스캔 대상: S&P 500 + 레버리지 ETF(TQQQ/SOXL 등) + 인기 미국 주식(MSTR/COIN/MARA 등) 550개 이상. ATR 기반 목표가(×2)/손절가(×1), 보유 당일
...
│   └── sp500_tickers.py         # S&P 500 종목 스크래핑 + 레버리지 ETF/인기 주식 리스트
├── data/
...
> **참고**: tracking data 커밋 step은 `if: always()` 조건으로 설정되어 있어 Slack 발송이 실패하더라도 추천/평가 데이터는 항상 커밋됩니다.
...
10. **Ross Cameron 추천** — RSI < 30 + MACD 골든크로스 Warrior Trading 전략 (보유 당일, S&P 500 + 레버리지 ETF + 인기 주식 550개 스캔, 앰버 카드)
11. 장기 투자 추천 Top 3 — 추세 추종 전략 (보유 기간 1-3개월, 축소 감성 게이지)
...
4-2. **Ross Cameron Watchlist** — RSI < 30 + MACD 골든크로스 전략 기반 당일 관심 종목 (S&P 500 + 레버리지 ETF + 인기 주식 550개 스캔, 앰버 카드)
5. 전일 추천 종목 현황 (PM변동 + RSI/MACD/칼만예측가 오버레이)
6. 경제 캘린더
7. 실적 발표 캘린더
8. 최신 뉴스

프리마켓 기본 대상 종목은 시장 ETF + 11개 섹터 ETF + 전일 추천 종목 + 당일 실적 발표 종목으로 구성됩니다. 개장 급등 추천은 S&P 500 전체를 `yf.download()` 배치 스캔하여 PM 상승 종목을 빠르게 추출한 뒤, 해당 종목만 상세 분석합니다 (종목 유형 제한 없음). Ross Cameron Watchlist는 S&P 500 + 레버리지 ETF + 인기 미국 주식 550개 이상을 스캔하여 당일 데이트레이딩 후보를 선별합니다.

### 추천 전략 비교

| | 단기 추천 | 칼만 필터 추천 | 장기 추천 | 개장 급등 (Opening Surge) | **Ross Cameron** |
|---|---|---|---|---|---|
| **전략** | 평균 회귀 (Mean Reversion) | 칼만 예측가 상승 여력 | 추세 추종 (Trend Following) | PM 모멘텀 + 뉴스 촉매 + 기술적 돌파 | RSI 과매도 반등 (Warrior Trading) |
| **칼만 필터** | 단기 칼만 (1-step, 20일 SMA) | 단기 칼만 (1-step) | **장기 전용 칼만** (N-step, 50일 SMA) | 미사용 | 미사용 |
| **핵심 조건** | blended_target ≥ 현재가+2%, RSI 과매도 등 | blended_target > 현재가 | Kalman velocity > 0, ADX bullish | PM ≥ +1.5%, 8개 항목 가중 점수 | RSI < 30 + MACD 골든크로스 + MACD > 0 |
| **보유 기간** | 1-4주 | 2-4주 | 1-3개월 | 30분~1시간 | **당일** |
| **종목 수** | 1개 | 1개 | 최대 3개 | 최대 3개 | **최대 5개** |
| **목표가/손절가** | 칼만+볼린저 블렌딩 / ATR 기반 | 칼만+볼린저 블렌딩 / ATR 기반 | — | ATR 기반 동적 | **ATR×2 / ATR×1 (R:R 2:1)** |
| **스캔 유니버스** | S&P 500 | S&P 500 | S&P 500 | S&P 500 | **S&P 500 + 레버리지 ETF + 인기 주식 (550개+)** |
| **리포트** | 일일 | 일일 | 일일 | 프리마켓 | **일일 + 프리마켓 모두** |
| **자동 튜닝** | 지원 (min_samples=15) | 지원 | 지원 (min_samples=5) | 지원 (min_samples=10) | 미지원 (규칙 기반 고정) |
...
일일 리포트 (`main.py`) 실행 시:
1. **이전 추천 평가** — 보유 기간이 경과한 pending 종목(Enhanced/Kalman/Long-term/Opening Surge)의 실제 가격을 조회하여 win/loss/expired 판정. `yf.download(end=today+1일)`로 오늘 종가 포함
2. **오늘의 추천 기록** — 새 추천 종목을 진입가, 목표가, 손절가, `score_breakdown`과 함께 기록. **최근 5일 이내 동일 ticker+method 중복 기록 자동 스킵** (통계 왜곡 방지)
